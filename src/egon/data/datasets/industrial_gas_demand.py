...
def read_industrial_demand(scn_name, carrier):
    ...
    This function read the CH4 or H2 industrial demands downloaded
...
def delete_old_entries(scn_name):
    """
    Delete CH4 and H2 loads and load timeseries.
...
def insert_new_entries(industrial_gas_demand, scn_name):
    ...
    The following steps are executed:
      ...
      * Return of the dataframe still containing the time series column
    ...
    industrial_gas_demand : pandas.DataFrame
        ...
        the database with the time series.
...
def insert_industrial_gas_demand_egon100RE():
    ...
    steps:
      * Read the CH4 and the H2 industrial demand points in Germany with
        fonctions :py:func:`read_industrial_CH4_demand` and
        :py:func:`read_industrial_H2_demand`.
      * Identify and adjust the total industrial CH4 and H2 loads for Germany
        generated by PyPSA-Eur-Sec.
          * For the CH4 the timeserie used is the one from H2, because
            the industrial CH4 demand in the opendata.ffe database is 0.
          * For the test mode, the total values are obtained by
            evaluating the share of H2 demand in test region
            (NUTS1: DEF, Schleswig-Holstein) with respect to the H2
            demand in full Germany model (NUTS0: DE).
            The task has been outsourced to save processing cost.
      * Aggregate the demands with the same properties at the same bus.
      * Insert the loads in the database by executing :py:func:`insert_new_entries`.
      * Insert the time series associated to the loads in the database
        by executing :py:func:`insert_new_entries`.
...
def insert_industrial_gas_demand_time_series(egon_etrago_load_gas):
    """
    Insert list of industrial demand time series (one per NUTS3)
